...
import { getDepartmentStats } from '@/server/services/stats';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = await getDepartmentStats(session.user.departmentId, period);
    return res.status(200).json(result);
  } catch (err) {
    console.error(`[API] ${req.method} /api/stats/department:`, err);
